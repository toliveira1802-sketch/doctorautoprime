-- =====================================================
-- MIGRATION KOMMO - VERSÃO CORRIGIDA
-- =====================================================
-- Criado em: 2026-01-24
-- Descrição: Integração Kommo CRM (sem foreign keys problemáticas)
-- 
-- INSTRUÇÕES:
...
-- =====================================================
-- 2. MAPEAMENTO OS → LEAD (SEM FOREIGN KEY)
-- =====================================================
CREATE TABLE IF NOT EXISTS kommo_os_mapping (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    os_id UUID NOT NULL,  -- Removido REFERENCES para evitar erro
...
CREATE TABLE IF NOT EXISTS kommo_sync_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    os_id UUID,  -- Nullable, sem foreign key
...
SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE 'kommo%';
