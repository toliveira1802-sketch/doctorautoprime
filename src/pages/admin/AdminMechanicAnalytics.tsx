...
const AdminMechanicAnalytics = () => {
  ...
  const getRadarData = () => {
    const stats = getMechanicStats().filter(s => s.totalFeedbacks > 0).slice(0, 5);
    
    return [
      { metric: "Performance", ...Object.fromEntries(stats.map(s => [s.name, s.avgPerformance])) },
      { metric: "Pontualidade", ...Object.fromEntries(stats.map(s => [s.name, s.avgPunctuality])) },
      { metric: "Qualidade", ...Object.fromEntries(stats.map(s => [s.name, s.avgQuality])) },
    ];
  };
    ...
    <AdminLayout>
      <div className="p-4 md:p-6 space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="glass-card border-none">
            <CardHeader>
              <CardTitle className="text-lg">Comparativo por Habilidade</CardTitle>
            </CardHeader>
            <CardContent>
              {radarData.length === 0 || Object.keys(radarData[0]).length <= 1 ? (
                <p className="text-muted-foreground text-center py-12">
                  Nenhum dado disponível
                </p>
              ) : (
