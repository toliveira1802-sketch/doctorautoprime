...
import { 
  CalendarIcon, Plus, Save, RefreshCw, X, Check, Clock, 
...
export default function AdminAgendaMecanicos() {
    ...
    setSchedule(prev => ({
      ...
      [editingCell.mechanicId]: {
        ...
        [editingCell.hora]: {
          ...
          vehicle_plate: vehicle?.plate || plate,
        ...
        Object.entries(slots).forEach(([hora, slot]) => {
          if (slot.isNew) {
            newSlots.push({
              ...
              hora_fim: hora.startsWith('E') ? '18:00' : `${parseInt(hora.split(':')[0]) + 1}:00`,
  ...
  const getSlotStatusColor = (status: string, tipo: string) => {
    if (tipo === 'encaixe') return 'bg-warning/80 hover:bg-warning';
    switch (status) {
      case 'agendado': return 'bg-primary/80 hover:bg-primary';
      case 'em_andamento': return 'bg-violet/80 hover:bg-violet';
      case 'concluido': return 'bg-success/80 hover:bg-success';
    ...
    <AdminLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-violet" />
              ...
              <thead>
                <tr className="border-b">
                  ...
                  {HORARIOS_ENCAIXE.map(e => (
                    <th key={e} className="p-3 text-center font-medium text-sm text-warning">
                      {e}
                    </th>
                  ))}
                </tr>
                      ...
                      return (
                        <td key={e} className="p-1">
                          {isEditing ? (
                            ...
                            <div 
                              className={cn(
                                "relative group h-12 rounded flex items-center justify-center text-xs font-bold text-white cursor-pointer bg-warning/80 hover:bg-warning"
                              )}
                            >
                              {slot.vehicle_plate?.slice(-4) || '???'}
                              <button
                            ...
                            <div 
                              className="h-12 rounded bg-warning/10 hover:bg-warning/20 border border-dashed border-warning/30 flex items-center justify-center cursor-pointer transition-colors"
                              onClick={() => handleCellClick(mechanic.id, e)}
                            >
                              <Plus className="w-4 h-4 text-warning/50" />
                            </div>
                          )}
                        </td>
                      );
