...
import { 
  ...
  AlertTriangle, TrendingUp, Filter, Eye
} from "lucide-react";
...
export default function AdminOperacional() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch appointments with vehicles (simulating workflow data)
      ...
      // Map appointments to workflow etapas
      ...
      appointments?.forEach((apt: any) => {
        if (!apt.vehicles) return;
        
        const etapaOrdem = statusToEtapa[apt.status] || 2;
        const etapa = etapasData?.find(e => e.ordem === etapaOrdem);
        
        if (etapa) {
          const daysInStage = differenceInDays(new Date(), new Date(apt.created_at));
          
          grouped[etapa.id] = grouped[etapa.id] || [];
          grouped[etapa.id].push({
            id: apt.vehicles.id,
            plate: apt.vehicles.plate,
            model: apt.vehicles.model,
            brand: apt.vehicles.brand,
            status: apt.status,
            mechanic_name: apt.mechanics?.name || null,
            client_name: apt.profiles?.full_name || null,
      ...
      if (config) {
        setCapacidade(prev => ({ ...prev, maxima: config.capacidade_maxima }));
      }
  ...
  const getCapacidadeStatus = () => {
    ...
    if (percentage <= 75) return { color: 'bg-success', label: 'CAPACIDADE OK', textColor: 'text-success' };
    if (percentage <= 100) return { color: 'bg-warning', label: 'ATENÇÃO', textColor: 'text-warning' };
  ...
  const getDaysBadgeColor = (days: number) => {
    if (days <= 2) return 'bg-success/20 text-success';
    if (days <= 5) return 'bg-warning/20 text-warning';
